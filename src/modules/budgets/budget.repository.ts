...
import {
  BudgetEntity,
  BudgetWithDetailsEntity,
} from "@/modules/budgets/budget.entities";
...
const BudgetRepository = {
  findById: async (
    id: string,
    userId: string,
  ): Promise<BudgetWithDetailsEntity | null> => {
    const { rows } = await db.query(
      `${budgetSelect} WHERE b.id = $1 AND b.user_id = $2`,
      [id, userId],
    );

    return (rows[0] as BudgetWithDetailsEntity | undefined) ?? null;
  },

  findManyByUserId: async (
    userId: string,
    query: BudgetsQueryDTO,
  ): Promise<BudgetWithDetailsEntity[]> => {
    ...
    const { rows } = await db.query(
      `${budgetSelect} WHERE ${conditions.join(" AND ")} ORDER BY b.year DESC, b.month DESC, c.name ASC`,
      values,
    );

    return rows as BudgetWithDetailsEntity[];
  },

  findByUniqueKey: async (
    userId: string,
    categoryId: string,
    month: number,
    year: number,
    ...
    const values: Array<string | number> = [userId, categoryId, month, year];
    const conditions = [
      `user_id = $1`,
      `category_id = $2`,
      `month = $3`,
    ...
    return (rows[0] as BudgetEntity | undefined) ?? null;
  },

  calculateSpent: async (
    userId: string,
    categoryId: string,
    month: number,
    year: number,
  ...
  insert: async (
    userId: string,
    data: CreateBudgetDTO & { id: string; spent: string },
  ): Promise<BudgetWithDetailsEntity> => {
    await db.query(
      `
  ...
  update: async (
    id: string,
    userId: string,
    data: UpdateBudgetDTO & { spent?: string },
  ): Promise<BudgetWithDetailsEntity | null> => {
